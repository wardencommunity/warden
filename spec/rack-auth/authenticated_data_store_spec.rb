...
describe "authenticated data store" do
  ...
  it "should store data for the default scope" do
    app = lambda do |e|
      ...
      e['rack-auth'].session[:key] = "value"
      valid_response
    end
    setup_rack(app).call(@env)
    @env['rack.session']['rack-auth.user.default.session'].should == {:key => "value"}
    @env['rack.session']['rack-auth.user.foo.session'].should be_nil
  end
  
  it "should store data for the foo user" do
    app = lambda do |e|
      e['rack-auth'].session(:foo)[:key] = "value"
      valid_response
    end
    setup_rack(app).call(@env)
    @env['rack.session']['rack-auth.user.foo.session'].should == {:key => "value"}
  end
  
  it "should store the data seperately" do
    app = lambda do |e|
      e['rack-auth'].session[:key] = "value"
      e['rack-auth'].session(:foo)[:key] = "another value"
      valid_response
    end
    setup_rack(app).call(@env)
    @env['rack.session']['rack-auth.user.default.session'].should == {:key => "value"}
    @env['rack.session']['rack-auth.user.foo.session'    ].should == {:key => "another value"}
  end
  ...
  it "should clear out the default data when :default logs out" do
    ...
    setup_rack(app).call(@env)
    @env['rack.session']['rack-auth.user.default.session'].should be_nil
    @env['rack.session']['rack-auth.user.foo.session'    ].should == {:key => "another value"}
  end
  ...
  it "should logout multuiple personas at once" do
    ...
    app = lambda do |e|
      e['rack-auth'].session[:key] = "value"
      e['rack-auth'].session(:foo)[:key] = "another value"
      e['rack-auth'].session(:bar)[:key] = "yet another"
    ...
    @env['rack.session']['rack-auth.user.default.session'].should be_nil
    @env['rack.session']['rack-auth.user.foo.session'    ].should == {:key => "another value"}
    @env['rack.session']['rack-auth.user.bar.session'    ].should be_nil
  end
  
  it "should not store data for a user who is not logged in" do
    @env['rack.session']
    app = lambda do |e|
      e['rack-auth'].session(:not_here)[:key] = "value"
      valid_response
    end
